...
const Portfolio = () => {
  ...
  useEffect(() => {
    ...
    if (costBasisList.length > 0) {
      dashboardService
        .getQuotes(costBasisList.map((costbasis) => costbasis.symbol))
        .then((json) => {
  ...
  useEffect(() => {
    ...
    if (quoteDtos.length > 0 && costBasisList.length > 0) {
      ...
      tempTotalAssetValue =
        Math.round((tempTotalAssetValue + Number.EPSILON) * 100) / 100;
  ...
  return (
    <div className="Portfolio">
      <div className="Title">
        <h1>Portfolio</h1>
        {!showSimulate && (
          <button className="LotListViewButton" onClick={toggleSimulate}>
            Simulate
          </button>
        )}
        {showSimulate && (
      ...
      {!showSimulate && (
        <PortfolioCostBasisList
          costBasisList={costBasisList}
          quoteDtos={quoteDtos}
