  ...
  useEffect(() => {
    if (quoteDtos && quoteDtos.length > 0) {
      const newIputFields = costBasisList.map(costBasis => {
        const quote = quoteDtos.filter(quote => quote.symbol === costBasis.symbol)[0];
        return {
...
const CostBasisCard = ({ simulationField, totalAssetValue, setIsUpdateAssetValue }) => {
  const [simulatedTotalShares, setSimulatedTotalShares] = useState(simulationField.simulatedTotalShares);
  const [currentPrice, setCurrentPrice] = useState(simulationField.currentPrice ? simulationField.currentPrice : 0);

  useEffect(() => {
    simulationField.simulatedTotalShares = simulatedTotalShares;
    simulationField.currentPrice = currentPrice;
    setIsUpdateAssetValue(true);
  }, [simulatedTotalShares, currentPrice, setIsUpdateAssetValue, simulationField]);
      ...
      <div className='CostBasis'>
        <div className='CostBasisHeader'>
          ...
          <div className='CostBasisValues'>
            <div className='Input'>
              <FieldInput field={'Shares'} fieldValue={simulatedTotalShares}
                setFieldValue={setSimulatedTotalShares} originalValue={simulationField.totalShares}
                className='shares-input' withCounter={true} />
              <FieldInput field={'Current Price'} fieldValue={currentPrice}
                setFieldValue={setCurrentPrice} originalValue={currentPrice}
                className='price-input' withCounter={false} />
            </div>
          </div>
          <div className='Simulated'>
            <FieldValue field={'Simulated Total Value'} value={getCurrentValue(simulatedTotalShares, currentPrice)} />
          </div>
          <div className='Simulated'>
            <FieldValue field={'Simulated Diversification'} value={getPercent(simulatedTotalShares, currentPrice)} />
          </div>
        </div>
      </div>
    </>
...
const FieldInput = ({ field, fieldValue, originalValue, setFieldValue, className, withCounter }) => {
    ...
    <div className='FieldInput'>
      ...
      <div className='Value'>
        <input
          ...
          className={className}
        />
        {sign && <span className={sign}>
          ({fieldValue - originalValue})
        </span>}
        {withCounter && <div className={'CounterButtons'}>
